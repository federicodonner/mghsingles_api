...
router.get("/:page", [upload.none(), check("page").isNumeric()], (req, res) => {
...
router.get(
  ...
  (req, res) => {
    ...
    let query = db.query(sql, (err, quantity) => {
      ...
      // Get the cards from active collections
      sql =
        "SELECT c.id, c.scryfallId, c.quantity, c.foil, n.name AS 'condition', l.name AS 'language', g.name AS 'cardName', g.cardSetName, g.image FROM card c LEFT JOIN collection o ON c.collectionId = o.id LEFT JOIN cardLanguage l ON c.languageId = l.id LEFT JOIN cardCondition n ON c.conditionId = n.id LEFT JOIN cardGeneral g ON c.scryfallId = g.scryfallId WHERE o.active = 1 AND g.name like '%" +
        cardName +
        "%' ORDER BY g.name";
      query = db.query(sql, (err, cards) => {
        if (err) {
